...
func CreateModifyConfigurationFile(configFile string) {
  ...
  var setCredentials = func(configFile string)(err error) {
    ...
    err, config := loadJsonFileToMap(configFile)
  ...
  var addLineup = func(configFile string)(err error) {
    ...
    if _, ok := menu[input]; ok {
      ...
      // Select headends
      err, headends := sdHeadends(parameter)
      if err != nil {
        return err
      }
      plz:
  ...
  var manageChannels = func(response SD_Status)(err error) {
    ...
    for _, r := range response.Lineups {
      ...
      err, channelList := sdChannelList(r.Lineup)
      ...
      for _, name := range channelNames {
        ...
        if none == false && all == false {
          ...
          switch(strings.ToLower(input)) {
            ...
            case "y": addChannel(channel)
            case "n": delete(myChannels, stationID);
            case "all": all = true;
            case "none": none = true;
            case "skip": skip = true;
      ...
      for _, c := range channelList.Stations {
        ...
        if all == false && none == false {
          ...
          if _, ok := myChannels[c.StationID]; ok {
            fmt.Printf("[+] [%s] Channel: %s %s\n", c.StationID, c.Name, c.BroadcastLanguage)
          } else {
            fmt.Printf("[-] [%s] Channel: %s %s\n", c.StationID, c.Name, c.BroadcastLanguage)
          }
        ...
        if none == false && all == false {
          ...
          switch(strings.ToLower(input)) {
            ...
            case "y": addChannel(channel)
            case "n": delete(myChannels, c.StationID);
            case "all": all = true;
            case "none": none = true;
            case "skip": skip = true;
  ...
  login:
  err, content := readByteFromFile(configFile)
  if err != nil {
    checkErr(err, true)
  }
...
func GetData(configFile string) {
  ...
  var addProgramsToCache = func(response SD_Programs)(err error) {
    ...
    err, cache := loadJsonFileToMap(cacheFile)
    if err != nil {
      return
    }
    //var md5       = cache["md5"].(map[string]interface{})
  ...
  var getSchedulesWithStationIDs = func()(err error) {
    ...
    for _, id := range myChannels {
      ...
      if count == downloadLimit {
        ...
        err, response := sdGetSchedules(mapToJson(sdChannels))
        if err != nil {
          return err
        }
  ...
  var getDataWitchProgramIDs = func(dataType string)(err error) {
    ...
    for key, _ := range schedules {
      ...
      for _, program := range g2g_Programs {
        ...
        switch(dataType) {
          case "metadata":  programID = program.ProgramID[0:10]
          case "programs":  programID = program.ProgramID
        }
        ...
        if count == downloadLimit {
          ...
          switch(dataType) {
            ...
            case "metadata":  
              ...
              err, response := sdGetMetadata(mapToJson(sdPrograms))
              if err != nil {
                return err
              }
            ...
            case "programs":   
              ...
              err, response := sdGetPrograms(mapToJson(sdPrograms))
              if err != nil {
                return err
              }
    ...
    switch(dataType) {
      ...
      case "programs": logInfo(AppName, fmt.Sprintf("New program data: %d", newData))
      case "metadata": logInfo(AppName, fmt.Sprintf("New metadata: %d", newData))
...
func checkErr(err error, exit bool) {
  ...
  switch runtime.GOOS {
    ...
    case "windows": log.Println(logMsg)
